...
function addBookToCompleted(selectedBookElement){
    const listCompleted = document.getElementById(COMPLETED_LIST_BOOK_ID);
    const bookTitle = selectedBookElement.querySelector(".book-desc h3").innerText;
    const bookAuthor = selectedBookElement.querySelector(".book-desc p:nth-child(2) span").innerText;
    const bookYear = selectedBookElement.querySelector(".book-desc p:nth-child(3) span").innerText;

    const newBook = createBook(bookTitle, bookAuthor, bookYear, true);
    const book = findBook(selectedBookElement[BOOK_ITEMID]);
    book.isComplete = true;
    newBook[BOOK_ITEMID] = book.id;

    listCompleted.append(newBook);
    selectedBookElement.remove();
...
function createCompletedButton(){
    return createButton("completed-button", function(event){
        const selectedBookElement = event.target.parentElement.parentElement;
        addBookToCompleted(selectedBookElement);
    });
}
